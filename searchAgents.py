...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty] # returns true or false
            if not hitsWall:
                nextPos = (nextx, nexty)
                tup = list()
                for i in state[1]:
                    if i != nextPos:
                        tup.append(i)
                myTup = tuple(tup)
                print("tup of next possible states: ", myTup)
                nState = (nextPos, myTup)
                print("nextstate (nextPos, tup):", nState)
                cost = self.costFn(nextPos)
                successors.append((nState, action, cost))
